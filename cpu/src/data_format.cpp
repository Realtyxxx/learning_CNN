...
void Tensor3D::normalize(const std::vector<data_type> mean, const std::vector<data_type> std_div) {
  ...
  for (int ch = 0; ch < C; ++ch) {
    ...
    for (int i = 0; i < ch_size; ++i) ch_ptr[i] = (ch_ptr[i] - mean[ch]) / std_div[ch];
  }
}
...
cv::Mat Tensor3D::opecv_mat(const int CH) const {
  ...
  if (CH == 3) {
    ...
    const int length = H * W;
    for (int i = 0; i < length; ++i) origin.data[i] = cv::saturate_cast<uchar>(255 * data[i]);
  }
  return origin;
}
...
std::shared_ptr<Tensor3D> Tensor3D::rot180() const {
  ...
  for (int c = 0; c < C; ++c) {
    ...
    for (int i = 0; i < ch_size; ++i) ch_ptr[i] = old_ptr[ch_size - 1 - i];
  }
  return rot;
}
...
std::shared_ptr<Tensor3D> Tensor3D::pad(const int padding) const {
  ...
  for (int c = 0; c < C; ++c)
    for (int i = 0; i < H; ++i)
      std::memcpy(padded->data + c * ch_size + (padding + i) * new_W + padding, this->data + c * H * W + i * W,
                  W * sizeof(data_type));
  return padded;
}
