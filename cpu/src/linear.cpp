...
std::vector<tensor> LinearLayer::backward(std::vector<tensor>& delta) {
  ...
  for (int i = 0; i < in_channels; ++i) {
    ...
    for (int j = 0; j < out_channels; ++j) {
      data_type sum_value = 0;
      for (int b = 0; b < batch_size; ++b) sum_value += this->__input[b]->data[i] * delta[b]->data[j];
...
void LinearLayer::save_weights(std::ofstream& writer) const {
  writer.write(reinterpret_cast<const char*>(&weights[0]),
               static_cast<std::streamsize>(sizeof(data_type) * in_channels * out_channels));
