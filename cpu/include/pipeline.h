...
std::map<std::string, list_type> get_images_for_classification(const std::filesystem::path    dataset_path,
...
class ImageAugmentor {
 private:
  std::default_random_engine e, l, c,
      r;  // e 用来获得操作的概率; l 用来打乱操作列表; c 用来得到裁剪需要的概率; r 用来得到旋转的概率
 ...
 public:
  ImageAugmentor(const std::vector<std::pair<std::string, float> >& _ops = {{"hflip", 0.5},
                                                                            {"vflip", 0.2},
                                                                            {"crop", 0.7},
                                                                            {"rotate", 0.5}})
      : e(212),
...
class DataLoader {
 ...
 public:
  explicit DataLoader(const list_type& _images_list, const int _bs = 1, const bool _aug = false,
                      const bool _shuffle = true, const std::tuple<int, int, int> image_size = {224, 224, 3},
                      const int _seed = 212);
  int        length() const;
