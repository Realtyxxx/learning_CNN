...
BatchNorm2D::BatchNorm2D(std::string _name, const int _out_channels, const data_type _eps, const data_type _momentum)
    : Layer(_name),
      ...
      eps(_eps),
      momentum(_momentum),
      gamma(_out_channels, 1.0),
      beta(_out_channels, 0),
      moving_mean(_out_channels, 0),
      moving_var(_out_channels, 0),
      buffer_mean(_out_channels, 0),
      buffer_var(_out_channels, 0) {
}
...
std::vector<tensor> BatchNorm2D::forward(const std::vector<tensor>& input) {
  ...
  if (this->output.empty()) {
    ...
    for (int b = 0; b < batch_size; ++b)
      this->output.emplace_back(new Tensor3D(out_channels, H, W, this->name + "_output_" + std::to_string(b)));
    this->normed_input.reserve(batch_size);
    for (int b = 0; b < batch_size; ++b)
      this->normed_input.emplace_back(new Tensor3D(out_channels, H, W, this->name + "_normed_" + std::to_string(b)));
  }
  // 记录输入
  if (not no_grad)
    this->__input = input;
  ...
  for (int o = 0; o < out_channels; ++o) {
    ...
    if (not no_grad) {
      ...
      data_type u = 0;
      for (int b = 0; b < batch_size; ++b) {
        data_type* const src_ptr =
            input[b]->data + o * feature_map_length;  // 现在找到了第 o 个输出的第 b 张图像的相关指针
        for (int i = 0; i < feature_map_length; ++i) u += src_ptr[i];
      }
      // 如果有 backward, 记住均值
      u = u / output_length;
      // 求方差 sigma
      ...
      for (int b = 0; b < batch_size; ++b) {
        ...
        for (int i = 0; i < feature_map_length; ++i) var += square(src_ptr[i] - u);
      }
