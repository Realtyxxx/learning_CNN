...
Conv2D::Conv2D(std::string _name, const int _in_channels, const int _out_channels, const int _kernel_size,
               const int _stride)
    : Layer(_name),
      bias(_out_channels),
  ...
  for (int o = 0; o < out_channels; ++o) {
    ...
    for (int i = 0; i < params_for_one_kernel; ++i) data_ptr[i] = engine(this->seed) / random_times;
  }
}
...
std::vector<tensor> Conv2D::backward(std::vector<tensor>& delta) {
  ...
  if (this->weights_gradients.empty()) {
    ...
    for (int o = 0; o < out_channels; ++o)
      this->weights_gradients.emplace_back(
          new Tensor3D(in_channels, kernel_size, kernel_size, this->name + "_weights_gradients_" + std::to_string(o)));
    // bias
      ...
      for (int i = 0; i < in_channels; ++i) {
        ...
        for (int k_x = 0; k_x < kernel_size; ++k_x) {
          for (int k_y = 0; k_y < kernel_size; ++k_y) {
            ...
            w_ptr[k_x * kernel_size + k_y] +=
                sum_value / batch_size;  //? OIHW , 统计每个图片的该channel weight值和他有关的的相关的
          }
        }
      }
      // 计算 b 的梯度
...
void Conv2D::update_gradients(const data_type learning_rate) {
  ...
  for (int o = 0; o < out_channels; ++o) {
    ...
    // 逐个卷积核做权值更新
    for (int i = 0; i < params_for_one_kernel; ++i) w_ptr[i] -= learning_rate * wg_ptr[i];
    // 更新 bias
...
void Conv2D::load_weights(std::ifstream& reader) {
  ...
  reader.read((char*)(&bias[0]),
              static_cast<std::streamsize>(sizeof(data_type) * out_channels));  // bias 只有out channel长度
}
