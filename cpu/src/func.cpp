...
inline data_type __exp(const data_type x) {
  if (x >= 88)
    return FLT_MAX;  // 直接返回 float 的最大值, 如果 data_type 换成 double 这个还得改
  else if (x <= -50)
    return 0.f;
  return std::exp(x);
}
...
std::vector<tensor> one_hot(const std::vector<int>& labels, const int num_classes) {
  ...
  for (int b = 0; b < batch_size; ++b) {
    ...
    for (int i = 0; i < num_classes; ++i) sample->data[i] = 0;
...
std::pair<data_type, std::vector<tensor> > cross_entroy_backward(const std::vector<tensor>& probs,
