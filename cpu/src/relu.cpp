...
std::vector<tensor> ReLU::forward(const std::vector<tensor>& input) {
  ...
  if (output.empty()) {
    ...
    for (int b = 0; b < batch_size; ++b)
      this->output.emplace_back(
          new Tensor3D(input[0]->C, input[0]->H, input[0]->W, this->name + "_output_" + std::to_string(b)));
  }
  // 只保留 > 0 的部分
  ...
  for (int b = 0; b < batch_size; ++b) {
    ...
    for (int i = 0; i < total_length; ++i) out_ptr[i] = src_ptr[i] >= 0 ? src_ptr[i] : 0;
  }
  return this->output;
}

std::vector<tensor> ReLU::backward(
    std::vector<tensor>& delta) {  // 这个没有 delta_output, 因为形状一模一样, 可以减少一些空间使用, 但为了多态要统一
  // 获取信息
