...
int main() {
  ...
  assert(train_batch_size >= valid_batch_size and
         train_batch_size >= test_batch_size);             // 设计问题, train 的 batch 必须更大
  ...
  for (int iter = start_iters; iter <= total_iters; ++iter) {
    ...
    // 打印信息
    ++cur_iter;
    printf("\rTrain===> [batch %d/%d] [loss %.3f] [Accuracy %4.3f]", iter, total_iters, mean_loss / cur_iter,
           train_evaluator.get());
    // 更新一次信息
    if (iter % valid_inters == 0) {
      ...
      for (int s = 1; s <= samples_num; ++s) {
        ...
        printf("\rValid===> [batch %d/%d] [loss %.3f] [Accuracy %4.3f]", s, samples_num, mean_valid_loss / s,
               valid_evaluator.get());
      }
      printf("\n\n");
      // 保存模型
...

  {
    ...
    for (int s = 1; s <= samples_num; ++s) {
      ...
      printf("\rTest===> [batch %d/%d] [loss %.3f] [Accuracy %4.3f]", s, samples_num, mean_test_loss / s,
             test_evaluator.get());
    }
  }
  return 0;
