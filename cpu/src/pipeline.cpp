...
std::map<std::string, pipeline::list_type> pipeline::get_images_for_classification(
    const std::filesystem::path dataset_path, const std::vector<std::string> categories,
  ...
  for (int i = 0; i < categories_num; ++i) {
    ...
    for (const auto& iter : walker) all_images_list.emplace_back(iter.path().string(), i);
  }
  // 打乱图像列表
  ...
  results.emplace("test",
                  list_type(all_images_list.begin() + train_size, all_images_list.begin() + train_size + test_size));
...
DataLoader::DataLoader(const list_type& _images_list, const int _bs, const bool _aug, const bool _shuffle,
                       const std::tuple<int, int, int> image_size, const int _seed)
...
std::pair<tensor, int> DataLoader::add_to_buffer(const int batch_index) {
  ...
  if (this->iter == this->images_num) {  // 取到头了, 重新开始
    ...
    if (this->shuffle) {                 // 然后再一次打乱列表
      std::shuffle(this->images_list.begin(), this->images_list.end(),
                   std::default_random_engine(this->seed));  // std::cout << this->images_list[0].first << ", " <<
                                                             // this->images_list[0].second << std::endl;
    }
  }
  // 读取图像
  ...
  // 对图像做数据增强
  if (this->augment)
    this->augmentor.make_augment(origin);
  // resize, 必须在数据增强之后
