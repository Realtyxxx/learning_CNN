...
std::vector<tensor> Dropout::forward(const std::vector<tensor>& input) {
  ...
  if (this->sequence.empty()) {
    ...
    for (int b = 0; b < batch_size; ++b)
      this->output.emplace_back(new Tensor3D(out_channels, input[0]->H, input[0]->W));
  }
  // 打乱列表
  ...
  if (not no_grad) {
    ...
    for (int i = 0; i < out_channels; ++i) this->mask[i] = i >= selected_num ? this->sequence[i] : -1;
    ...
    for (int b = 0; b < batch_size; ++b) {
      for (int o = 0; o < out_channels; ++o) {  // 考察 o 通道
        if (o >= this->selected_num)            // 如果是保留的神经元
          ...
          std::memset(this->output[b]->data + o * area, 0, copy_size);
      }
    }
  } else {  // 验证或者测试阶段
