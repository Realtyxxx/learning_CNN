...
void test_dataloader() {
  ...
  for (int i = 0; i < 10; ++i) {
    ...
    for (int b = 0; b < train_batch_size; ++b) {
      std::cout << "[Batch " << i << "] "
                << " [" << b + 1 << "/" << train_batch_size << "]===> " << categories[labels[b]] << std::endl;
...
void test_relu_layer() {
  ...
  for (int i = 0; i < length; ++i) data_ptr[i] = engine(e);
  ...
  // 随机生成 delta 的数据
  for (int i = 0; i < length; ++i) delta[0]->data[i] = engine(e);
  // 打印所有梯度的内容
  delta[0]->print(CH);
...
void test_maxpool2d_layer() {
  ...
  for (int i = 0; i < length_2; ++i) delta[0]->data[i] = engine(e);
  // 打印所有梯度的内容
  delta[0]->print(CH);
